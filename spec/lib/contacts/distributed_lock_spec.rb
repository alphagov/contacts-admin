...
RSpec.describe "Contacts::DistributedLock" do
  describe "#lock" do
    let(:redis_instance) { instance_double(Redis, lock: {}) }
    ...
    it "yields the block when a redis lock can be acquired" do
      expect(redis_instance).to receive(:lock)
    ...
    it "silently doesn't a yield the block when a lock can't be acquired" do
      allow(redis_instance).to receive(:lock).and_raise(Redis::Lock::LockNotAcquired)
